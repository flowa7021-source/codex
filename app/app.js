...
function preprocessOcrCanvas(inputCanvas, thresholdBias = 0) {
  ...
  const threshold = Math.max(80, Math.min(190, mean + thresholdBias));
...
function scoreOcrTextByLang(text, lang) {
  ...
  if (lang === 'rus') return (cyr * 3) - lat + digits;
  if (lang === 'eng') return (lat * 3) - cyr + digits;
  ...
  if (lang === 'eng') {
    ...
    if (lat > cyr * 1.4 && /[а-яё]/i.test(out) === false) {
      out = out
        ...
        .replace(/B/g, 'В').replace(/E/g, 'Е').replace(/e/g, 'е')
