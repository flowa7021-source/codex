...
async function runOcrOnPreparedCanvas(canvas, options = {}) {
  ...
  const lowMemory = !!options.lowMemory;

  const preprocessStart = performance.now();
  const isAccurate = !lowMemory && state.settings?.ocrQualityMode === 'accurate';
  const recipeList = lowMemory
    ? [
      [0, 'otsu', false, 1],
      [-10, 'mean', false, 0.95],
    ]
  ...
  let skewToApply = 0;
  if (!lowMemory && Math.abs(preferredSkew) >= 0.35) {
    skewToApply = preferredSkew;
  } else if (!fast && !lowMemory) {
...
async function runOcrOnRectNow(rect) {
  ...
  try {
    ...
    let text = '';
    try {
      text = await runOcrOnPreparedCanvas(src, {
        preferredSkew,
        taskId,
      ...
      const fallback = constrainOcrSourceCanvasPixels(src, 1_600_000).canvas;
      pushDiagnosticEvent('ocr.manual.retry.low-memory', {
        taskId,
        page: state.currentPage,
        message: error?.message || String(error || 'unknown error'),
      }, 'warn');
      setOcrStatus('OCR: повтор в экономном режиме...');
      text = await runOcrOnPreparedCanvas(fallback, {
        preferredSkew: 0,
        taskId,
        fast: true,
        lowMemory: true,
    ...
    pushDiagnosticEvent('ocr.manual.error', { taskId, totalMs, page: state.currentPage, message: error?.message || String(error || 'unknown error') }, 'error');
  } finally {
...
async function renderCurrentPage() {
  ...
  saveViewState();
  pushDiagnosticEvent('page.render', { page: state.currentPage, zoom: Number(state.zoom.toFixed(2)), ms: Math.round(performance.now() - renderStartedAt) });
}
