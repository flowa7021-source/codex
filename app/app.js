...
function normalizeOcrTextByLang(text) {
  const lang = state.settings?.ocrLang || 'auto';
...
async function runOcrOnRect(rect) {
  ...
  try {
    ...
    prep.width = sw * upscale;
    prep.height = sh * upscale;
    const pctx = prep.getContext('2d');
    pctx.imageSmoothingEnabled = true;
    pctx.drawImage(src, 0, 0, prep.width, prep.height);
    const img = pctx.getImageData(0, 0, prep.width, prep.height);
    const d = img.data;
    ...
    const text = normalizeOcrTextByLang(window.OCRAD(prep));
    if (text) {
...
async function extractTextForPage(pageNumber) {
  ...
  try {
    ...
    return normalizeOcrTextByLang(window.OCRAD(canvas));
  } catch {
    return '';
  }
}
...
async function startBackgroundOcrScan() {
  ...
  const maxPages = Math.min(state.pageCount, 120);
...
function applyLayoutState() {
  ...
  if (els.toggleAnnotTools) els.toggleAnnotTools.textContent = `Аннотации-панель: ${annotToolsHidden ? 'off' : 'on'}`;
