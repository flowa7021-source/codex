...
const els = {
  ...
  cfgTabsHeight: document.getElementById('cfgTabsHeight'),
...
function applyUiSizeSettings() {
  ...
  const tabsHeight = Math.max(28, Math.min(72, Number(settings.uiTabsPx) || 34));
  ...
  document.documentElement.style.setProperty('--ui-tabs-height', `${Math.round(tabsHeight)}px`);
...
function morphologyCloseMonochrome(imageData) {
  ...
  const dilated = new Uint8ClampedArray(src);
  ...
  for (let y = 1; y < height - 1; y += 1) {
    for (let x = 1; x < width - 1; x += 1) {
      ...
      const outIdx = (y * width + x) * 4;
      dilated[outIdx] = dilated[outIdx + 1] = dilated[outIdx + 2] = maxVal;
  ...
  for (let y = 1; y < height - 1; y += 1) {
    for (let x = 1; x < width - 1; x += 1) {
      ...
      const outIdx = (y * width + x) * 4;
      data[outIdx] = data[outIdx + 1] = data[outIdx + 2] = minVal;
      data[outIdx + 3] = 255;
    }
  }
}
...
async function runOcrOnPreparedCanvas(canvas, options = {}) {
  ...
  if (Math.abs(preferredSkew) >= 0.35) {
    variants = variants.flatMap((v) => [v, rotateCanvas(v, -preferredSkew)]);
  } else if (!fast) {
...
function openSettingsModal() {
  ...
  if (els.cfgTabsHeight) els.cfgTabsHeight.value = String(state.settings?.uiTabsPx || 34);
...
function readUiSizeSettingsFromModal() {
  return {
    ...
    tabsHeight: Math.max(28, Math.min(72, Number(els.cfgTabsHeight?.value) || 34)),
...
function previewUiSizeFromModal() {
  ...
  document.documentElement.style.setProperty('--ui-tabs-height', `${Math.round(values.tabsHeight)}px`);
...
function saveSettingsFromModal() {
  ...
  if (els.cfgTabsHeight) state.settings.uiTabsPx = Math.max(28, Math.min(72, Number(els.cfgTabsHeight.value) || 34));
...
async function runOcrOnRect(rect) {
  ...
  try {
    ...
    const text = await runOcrOnPreparedCanvas(src, { preferredSkew, taskId });
...
async function openFile(file, options = {}) {
  ...
  if (fromTabSwitch && tabId) restoreTabSnapshot(tabId);
  stopReadingTimer(false);
...
function saveActiveTabSnapshot() {
  if (!state.activeTabId || !state.docName) return;
  state.tabSnapshots[state.activeTabId] = {
    docName: state.docName,
...
function restoreTabSnapshot(tabId) {
  if (!tabId) return;
  const snap = state.tabSnapshots[tabId];
  if (!snap || snap.docName !== state.docName) return;
  state.currentPage = Math.max(1, Math.min(state.pageCount || 1, Number(snap.currentPage) || 1));
  state.zoom = Math.max(0.3, Math.min(4, Number(snap.zoom) || 1));
  state.rotation = [0, 90, 180, 270].includes(Number(snap.rotation)) ? Number(snap.rotation) : 0;
  if (els.searchInput) els.searchInput.value = snap.searchQuery || '';
}
...
function renderTabs() {
  ...
  state.openTabs.forEach((tab) => {
    ...
    wrap.className = `doc-tab${tab.id === state.activeTabId ? ' active' : ''}`;

    const main = document.createElement('button');
    main.type = 'button';
    main.title = tab.name;
    main.textContent = tab.name;
    ...
    close.addEventListener('click', async (e) => {
      ...
      delete state.tabSnapshots[tab.id];
    ...
    els.docTabsList.appendChild(wrap);
  });
}
...
function setupResizableLayout() {
  ...
  if (els.canvasResizeHandle) {
    ...
    const onMove = (e) => {
      ...
      if (!viewerArea || !els.canvasWrap) return;
...
els.cfgTabsHeight?.addEventListener('input', previewUiSizeFromModal);
